%Code for Solving Inhomogenous DE using Variation of Parameters and IVP -
%Illustrations taken from Theory Classes - Conditions supplied randomly
...
Wd = det(Wm);
if (Wd==0)
    y2 = y2*x;    %Taking care if the Functions are Dependent
end
Wm = [y1 y2;diff(y1,x) diff(y2,x)];
Wd = det(Wm);
yc = c1*y1 + c2*y2;
...
disp(['The solution of the Given Inhomogenous Differential Equation/IVP is: ',char(10),sym2str(y_final)])